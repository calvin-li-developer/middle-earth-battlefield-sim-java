...
/**
 * <i>Army</i> class is used to create armies with arrays of <i>Actor</i> subclasses using the ArrayList.
 *
...
public class Army
{
    ...
    //INSTANCE FIELDS
    /**
     * <i>name</i> is the instance field used to give a name to the army that is created.
     */
    private String name;
    /**
     * <i>collectionActors</i> is a reference-to an ArrayList Object. It is used to store various <i>Actor</i> subclass objects.
     */
    ...
    /**
     * <i>Army</i> constructor. It is used to automatically create an <i>Army</i>. It takes a parameter <i>name</i> to identify the <i>Army</i> created.
     *
     * @param name      Accepts a string input from the user to give the <i>Army</i> a name.
     * @param simulator
     */
    ...
    /**
     * The <i>populate</i> method is used to fill up the <i>Army</i> object that is created.
     * It takes two parameters <i>type</i> and <i>numToAdd</i>.
     *
     * @param type     This is used to declare what type of <i>Actor</i> subclass object is being used.
     * @param numToAdd This is used to declare how many instances are to be created to fill up the ArrayList.
     */
    ...
    public void populate(ActorFactory.Type type, int numToAdd)
    {
        for (int i = 0; i < numToAdd; ++i)
        {
            ...
            if (type.equals(type.HOBBIT) || type == type.WIZARD)
            {
                ...
                double newY = this.simulator.getScene().getHeight() / standDeviY + (this.ARMYGROUPRATIO * Math.random());
            ...
            } else
            {
                ...
                double newY = this.simulator.getScene().getHeight() / standDeviY + (this.ARMYGROUPRATIO * Math.random());//340
                avatar.setTranslateX(newX);
                avatar.setTranslateY(newY);
                avatar.setEffect(this.dropShadow);
                this.simulator.getChildren().add(actor.getAvatar());
    ...
    /**
     * The <i>displayIndividualActor</i> method is used to return a specific <i>Actor</i> object from the <i>Army</i> it belongs in.
     * It takes one parameter <i>indexOfActorToEdit</i>.
     *
     * @param indexOfActorToEdit This is used to look up the <i>Actor</i> in the ArrayList using the index.
     * @return Actor
     */
    ...
    /**
     * This is used to display all the Actors and their stats that belong to the <i>Army</i> object.
     *
    ...
    /**
     * This method is used to return the length of the <i>Army</i> ArrayList in an int value.
     *
     * @return int
     */
    public int size()
    ...
    /**
     * This method is used to make changes to an existing <i>Actor</i> subclass object in the Army.
     * It takes one parameter <i>indexOfActorToEdit</i>.
     *
     * @param indexOfActorToEdit This is used to look up the <i>Actor</i> in the ArrayList using the index.
     */
    ...
    /**
     * get an opposing army for different armyAllegiance
     */
     ...
     *
     * @param opposingArmy other side force
     */
    ...
    /**
     * this is use when actors drop below 1 health so they can get removed and displays their death on screen
     *
     * @param nowDeadActor
     */
    public void removeNowDeadActor(Actor nowDeadActor)
    {
        ...
        { // setup Stack Frame to allow re-use of variable identifiers "tt", "ft" and "pt"
            ...
            ft.setToValue(0.0); // no need to associate with the Text (message) here, that will be done in the ParallelTransition
            TranslateTransition tt = new TranslateTransition(duration);
            tt.setByY(200.0);  // no need to associate with the Text (message) here, that will be done in the ParallelTransition
        ...
        { // Army has been wiped out, since no Actor objects remain in the collection. Therefore . . . the opposing Army wins.
