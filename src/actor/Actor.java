...
public abstract class Actor implements Serializable
{
    ...
    /**
     * Stores the <i>strength</i> value assigned to the specific <i>Actor</i> * object
     */
    private SimpleDoubleProperty strength = new SimpleDoubleProperty();
    /**
     * Stores the <i>speed</i> value assigned to the specific <i>Actor</i> * object
     */
    private SimpleDoubleProperty speed = new SimpleDoubleProperty();
    /**
     * Stores the <i>health</i> value assigned to the specific <i>Actor</i> * object
     */
    ...
    public Actor()
    {
        setName(this.getClass().getSimpleName() + String.format("%02d", actorSerialNumber));
        setStrength((double) Math.round((SingletonRandom.instance.getNormalDistribution(MIN_STRENGTH, MAX_STRENGTH, 3.0)) * 10.0) / 10.0);
        setHealth((double) Math.round((SingletonRandom.instance.getNormalDistribution(MIN_HEALTH, MAX_HEALTH, 3.0)) * 10.0) / 10.0);//Math.round((MAX_HEALTH)*10)/10.0);
        setSpeed((double) Math.round((SingletonRandom.instance.getNormalDistribution(MIN_SPEED, MAX_SPEED, 3.0)) * 10.0) / 10.0);
        actorSerialNumber++;
    }
    ...
    public static TableView<Actor> createTable()
    {
        ...
        final double PREF_WIDTH_DOUBLE = 82.5;
        ...
        healthCol.setResizable(false);
        healthCol.setStyle("-fx-alignment: CENTER;");
        ...
        strengthCol.setResizable(false);
        strengthCol.setStyle("-fx-alignment: CENTER;");
        ...
        locationXCol.setResizable(false);
        locationXCol.setStyle("-fx-alignment: CENTER;");
        ...
        locationYCol.setResizable(false);
        locationYCol.setStyle("-fx-alignment: CENTER;");
    ...
    /**
     * Allows user inputed values for <i>health</i> within max and min
     * boundaries that have been preset.
     *
    ...
    /**
     * Allows user inputed values for <i>strength</i> within max and min
     * boundaries that have been preset.
     *
     ...
     * entries for the individual attributes (<i>name</i>, <i>health</i>,
     * <i>strength</i> and <i>speed</i>), within the pre-specified ranges.
     */
    ...
    /**
     * This method starts a new round of combat between two different
     * <i>Actor</i> objects.
     *
     * @param defender is a reference to a different <i>Actor</i> object that will be
     ...
     */
    // This code has been obtained from author Professor Rex Woollard from his in-class lectures
    public String toString()
    {
        return String.format("Name: %-13sHealth: %-10.1fStrength: %-10.1fSpeed: %-10.1f ",
                this.getName(), this.getHealth(), this.getStrength(), this.getSpeed());
    }
    ...
    /**
     * startMoving method is method use to make avatars move and rotate
     * use for most of the displays of how actors interact in simulator
     */
    ...
    {
        if (this.tt != null && this.tt.getStatus() == Animation.Status.RUNNING && this.tt != null)
        {
            this.tt.stop();
        }
        if (this.rt != null && this.rt.getStatus() == Animation.Status.RUNNING && this.rt != null)
        {
            this.rt.stop();
        }
    }
    ...
    /**
     * this is use for actors that have less then 15.0 when actors are to weak and they will not move
     *
     * @return this will make then stay at the point therefore no more movements for that actor
     */
    ...
    /**
     * this is use to calculate the distance between the actor and the opposing actor
     *
    ...
    public void resetAvatarAttributes()
    {
        ...
        this.tooltip.setText(this.toString());// Note: This updates the text in the Tooltip that was installed earlier. We re-use the originally installed Tooltip.
    }
    ...
    /**
     * resume/open the saved actors and display coordinates for actors and their value of attributes
     *
     * @param in
     * @throws IOException
