...
public class FXLauncher extends Application
{
    ...
    private MenuBar createMenuBar()
    {
        ...
        populateMenuItem.setOnAction(event -> simulator.populate());    // create CALLBACK, that is, the code to execute when triggered by user event (in this case, simulator.populate())
        MenuItem runMenuItem = new MenuItem("_Run");
        runMenuItem.setOnAction(event -> simulator.run());                        // create CALLBACK, that is, the code to execute when triggered by user event (in this case, simulator.run())
        MenuItem suspendMenuItem = new MenuItem("_Suspend");
        suspendMenuItem.setOnAction(event -> simulator.suspend());        // create CALLBACK, that is, the code to execute when triggered by user event (in this case, simulator.suspend())
        Menu menuRun = new Menu("_Run");
        menuRun.getItems().addAll(populateMenuItem, runMenuItem, suspendMenuItem);    // assemble MenuItems in the "Run" Menu

        // Create the "Properties" Menu
        MenuItem openArmyListsMenuItem = new MenuItem("Show Army _Lists");
        openArmyListsMenuItem.setOnAction(event -> simulator.openListViewWindow());    // create CALLBACK, that is, the code to execute when triggered by user event (in this case, simulator.openListViewWindow())
        MenuItem openArmyTablesMenuItem = new MenuItem("Show Army _Tables");
        openArmyTablesMenuItem.setOnAction(event -> simulator.openTableViewWindow());    // create CALLBACK, that is, the code to execute when triggered by user event (in this case, simulator.openListViewWindow())
        Menu menuProperties = new Menu("_Properties");
        menuProperties.getItems().addAll(openArmyListsMenuItem, openArmyTablesMenuItem); // assemble MenuItems in the "Run" Menu
